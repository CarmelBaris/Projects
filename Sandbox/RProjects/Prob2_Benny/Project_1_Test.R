...
file_name <- "C:/Users/user/Documents/Carmel/RProjects/Prob2_Benny/318455276_project_1.R"
...
if (any(str_detect(script, c("rgeom", "rexp")))){
  pass <- FALSE
  stop("Your code contains `rgeom` or `rexp` ")
} else {
    print("Your code is free of rgeom or rexp :))")
}
...
if((length(X) != n) | (!is.numeric(X))){
  stop("ex1q2 does not produce a numeric vector of length n")
    ...
    print("ex1q2 produces a numeric vector of length n!")
}
...
if (d > 0.01){
  ...
  stop("The marginal distribution in ex1q1 is not geometric")
    ...
    print("The marginal distribution in ex1q1 is indeed geometric!")
}
