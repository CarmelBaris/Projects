...
#load data
print(getwd())
new_dir <- 'C:/Users/user/Documents/Carmel/RProjects/Regression_intro/Startups'
setwd(new_dir)
startups <- read.csv(paste0(new_dir, '/Startups.csv'))

#get to know the dataset
nrow(startups) # num of rows excluding header row, 36
colnames(startups) # read column names
head(startups) # read first 6 entries

#check for missing values in the datset
missing_values <- is.na(startups)
...
For each startup, the dataset lists its expenses on Research & Development, Administration & Marketing, as well as its Profit. Additionally, the dataset categorizes the startups based on the State the reside in.
...
```{r}
hist(startups$Profit, breaks = nrow(startups))
```
...
plot(startups$Administration, startups$Profit)
...
```{r}
cont_vars <- startups[,-4] # select all columns except the 4th column (`State`)
