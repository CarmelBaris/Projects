...
loadRData = function(fileName) {
  #' Helper for loading and returning file for storing as variable:
...
check_overlaps = function(train_indices, test_indices) {
  ...
  length(overlaps) == 0  # Should be TRUE if there are no overlaps between sets
}
...
check_data_size = function(train, test, original_data) {
  ...
  total_train_test == original_length  # Should be TRUE if data size maintained
}
