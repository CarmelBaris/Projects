...
eps = 1
corrected_1n = fix_med(fix_gc_e(reads_5K_n, preds1n[1:l_1n], eps))
uncorrected_1n = fix_med(fix_gc_e(reads_5K_n, rep(1,l_1n), eps))

# One Sample with espilon, Tumor
eps = 1
corrected_1t = fix_gc_e(reads_5K_t, preds1t[1:l_1t], eps)
uncorrected_1t = fix_gc_e(reads_5K_t, rep(1,l_1t), eps)

gc_med_corrected_1t = fix_med(fix_gc_e(reads_5K_t, preds1t[1:l_1t], eps))
gc_med_uncorrected_1t = fix_med(fix_gc_e(reads_5K_t, rep(1,l_1t), eps))
...
plot_fixes(gc_med_uncorrected_1t, yla = "Before")

mtext(side=3, line=0.35, cex=0.9, "Tumor Sample")

plot_fixes(gc_med_corrected_1t, yla = "After")
...
# Plot for Tumor
plot(gc_med_corrected_1t[range_cd], ylab = "Tumor",xaxt = "n",
     ylim = c(0,2), cex = 0.2, pch=20)
abline(h=1, col=4, lwd = 1.5)
...
```{r}
two_sample_w_gc = fix_med(fix_gc_e(corrected_1t, corrected_1n))
two_sample_w_gc_e = fix_med(fix_2samp_e(corrected_1t, corrected_1n, 0.01))
two_sample_wout_gc = fix_med(fix_2samp_e(uncorrected_1t, uncorrected_1n))
two_sample_wout_gc_e = fix_med(fix_2samp_e(uncorrected_1t, uncorrected_1n, 0.01))
...
trim_med_e_1t = trim(gc_med_uncorrected_1t[use_range],0,3)
hist(trim(gc_med_uncorrected_1t[use_range],0,3),
...
trim_med_gc_e_1t = trim(gc_med_corrected_1t[use_range],0,3)
...
# Add legend
legend(c(4,-4), legend = c("Expect Mean", "Actual Mean"),
       col = c(4, 2), lwd = 3,
       lty = 1, bty = "n")
...
abline(h=1,col=4)
plot(range_b,gc_med_corrected_1t[range_b],main = "GC (aka One Samp)",
     ylim = c(0,3.5),cex = 0.2,pch=20)
abline(h=1,col=4)
