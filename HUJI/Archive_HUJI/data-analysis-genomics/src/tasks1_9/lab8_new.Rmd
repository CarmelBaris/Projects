...
Find specific areas in the chromozome
```{r}
# find subset
...
# הגדרת פרמטרים לגרפים אחד מעל השני ושוליים
par(mfrow = c(2, 1), mar = c(4, 4, 2, 1))

# גרף ראשון
plot(corrected_1t[range_a], main = paste("Cancer - One Sample Correction (eps=", eps, ")"),
     ylab = "estimated a",
     ylim = c(0, 1.7), cex = 0.5, pch = 20)
abline(h = 1, col = "green", lw = 2)
...
# par(mfcol = c(2,1),mar = c(2,2,2,2))
plot(two_sample_with_gc[range_a],main = paste("Cancer - Two Sample Correction with GC (eps=",eps_2,")"),
     ylab = "estimated a",
     ylim = c(0,3.5),cex = 0.5,pch=20)
abline(h=1,col="green", lw= 2)

plot(two_sample_without_gc[range_a],main = paste("Cancer - Two Sample Correction without GC (eps=",eps_2,")"),
     ylab = "estimated a",
     ylim = c(0,3.5),cex = 0.5,pch=20)
abline(h=1,col="green", lw= 2)
```
...
snr_after <- mean(two_sample_with_gc) / sd(two_sample_with_gc[range_a])
...
Find area of interest in the chromozome
```{r}
