...
chr1_line = factor(chr1_ls, c("A","T","C","G")) #assigns int to each type
N = length(chr1_vec)
...
The following function receives a region of the reference genome, and cells of 1000 nucleobases each, and sums the number of times a given base appears in the 
...
```{r part3, fig.width=5, fig.height=5}
...
#create histograms
p1 = hist(A_count,col=rgb(1,0,0,0.25))
p2 = hist(T_count,col=rgb(0,0,1,0.25))
p3 = hist(C_count,col=rgb(0,0,1,0.25))
p4 = hist(G_count,col=rgb(0,0,1,0.25))

plot(p1,col=rgb(1,0,0,0.25))
plot(p2,col=rgb(0,0,1,0.25),add=T)

plot(p3,col=rgb(1,0,0,0.25))
plot(p4,col=rgb(0,0,1,0.25),add=T)
# plot(p1, col=rgb(0,0,1,0.25))  # first histogram
```
