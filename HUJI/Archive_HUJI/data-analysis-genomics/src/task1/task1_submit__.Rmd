...
This week the focus is on the prevalence of nucleobases in each location along a given chromosome section.
...
cat("unknown:", (mt)/N*100, "%\n") #coded as "N" in original string, treated as N/A
round(tb/1000000,2)
```
...
## Part 3: Distribution of Nucleobases
...
for (i in 1:n_bins){
  ...
  NA_count[i] = sum(is.na(subSnippet(start_indx = start_bin)))
...
## Part 4: Base Frequencies

The next graph shows the frequencies of four nucleotide bases (A, C, G, T) across different chromosome areas. Bases C (green) and G (cyan) have very similar lines, almost identical, with only minor differences, indicating a strong correlation in their distribution. Bases A (red) and T (purple) also show similar trends, but they have more noticeable differences in their frequencies, suggesting a moderate correlation. In summary, C and G are highly similar, while A and T are somewhat similar but with more variation.


```{r Base_Frequencies, message=TRUE}
...
## Part 5: Pair Frequencies

a. The scatter plot comparing the counts of C vs G in each location shows that they are fairly similar. This indicates a strong correlation between these two bases. 
A significant correlation can also be seen in the scatter plot of A vs T, although not as strong as that between C and G. This suggests that A and T bases frequently occur together. The correlation of all other pairs does not seem to be as significant.\n

b. In each scatter plot, outlier observations are marked in red. The outliers are calculated based on the mean and standard deviation of a dataset. Data points that are more than a 5 standard deviations away from the mean are considered significant outliers. \n

c. For significant outliers, the scatter plot includes labels indicating their location along the length of the chromosome. \n

```{r Pair_frequencies, warning=FALSE}
# extract vector of non-null unique bases
unique_bases <- unique(chr1_single)

# create a dataframe that counts each base
...
# for each bin sums the number of occurrences of each base
for (i in 1:n_bins) {
  start_bin <- 1 + (i - 1) * bin_size
...
# converts bin number to actual bin positions for plotting
base_counts$bin_position <- base_counts$bin * bin_size

# adds frequency and column to base_counts dataframe 
base_counts <- base_counts %>%
  group_by(bin, base) %>%
  mutate(frequency = count / bin_size) %>%
  ungroup()

# creates another dataframe for pair frequencies of each combination of bases
...
# filters out duplicate pairs (for example keep (A, T) but not (T, A))
...
# creates scatter plots for each unique pair with outliers highlighted
...
for (i in 1:nrow(unique_pairs)) {
  ...
  # identifies outliers with a stricter threshold (e.g., 4 standard deviations)
  plot_data <- plot_data %>%
...
It seems that the outliers are more prevalent at either side of the given chromosome snippet. This makes sense because that's the location of the empty values. If a nucleobase has not been recognized, its pair might look as if there is an imbalance, as seen in the scatter plots. \n
Note that on the left side, this aligns with what we know about the structure of the chromosome. Our snippet is relatively at the beginning of the chromosome i.e. closer to the telomer. Meaning the empty values occur most in places where that the sequencing struggled more.
