...
```{r}
# Define helper functions to calculate MSE and MAD
...
calculate_mse(mod_tumor$fitted.values, preds_tumor)
calculate_mse(mod_healthy$fitted.values, preds_healthy)
```
...
f = function(Y, X, deg=3, knots=c(0.415, 0.45, 0.54)) {
  fit = lm(Y ~ bs(X, degree = deg, knots = knots, Boundary.knots =range()))
  return(fit)
}

mod_2n = f(r_n2[subs_n], GC[subs_n])
mod_2t = f(r_t2[subs_t], GC[subs_t])

# mod_2n = lm(r_n2~bs(GC, degree = 3, knots = myknots, Boundary.knots = c(min(GC), max(GC))),subset = subs_n)
# mod_2t = lm(r_t2~bs(GC, degree = 3, knots = myknots, Boundary.knots = c(min(GC), max(GC))),subset = subs_t)


resp_n = predict(mod_2n, newdata = data.frame(X = pred_range))
preds2n = predict(mod_2n, newdata = data.frame(X = GC2.5K_clean))

resp_t = predict(mod_2t, newdata = data.frame(X = pred_range))
preds2t = predict(mod_2t, newdata = data.frame(X = GC2.5K_clean))

par(mfcol = c(1,2), mar = c(3,5,3,3))

plot(GC2.5K_clean[samp1], reads2.5K_n2_cl[samp1], ylim = c(0,350), xlim = c(0.2,0.75), cex = 0.5, pch=20, main = "Healthy 2")
lines(pred_range, resp_n, col = 4, lw=3)

plot(GC2.5K_clean[samp1], reads2.5K_t2_cl[samp1], ylim = c(0,350), xlim = c(0.2,0.75), cex = 0.5, pch=20, main = "Tumor 2")
...
plot(GC_healthy, reads2.5K_t2_cl, col = 'darkgray', pch = 20, cex = 0.5, ylab = "Normalized Reads", xlab = "GC Content", main = "Healthy vs Tumor Sample", ylim = c(0, 350))
