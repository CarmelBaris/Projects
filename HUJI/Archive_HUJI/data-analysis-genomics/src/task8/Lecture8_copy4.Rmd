...
mod_1n = lm(reads2.5K_n2~bs(GC,deg = 3,knots = c(seq(0.25,0.6,0.025),0.65), Boundary.knots = c(min(GC),max(GC))),subset = subs_n)
...
mod_1t = lm(reads2.5K_t2~bs(GC,deg = 3,knots = knots_seq),subset = subs_t)
...
```{r}
# take maximum with 0
preds1n = pmax(predict(mod_1n,list(GC=GC2.5K)),0)
preds1t = pmax(predict(mod_1t,list(GC=GC2.5K)),0)


# One Sample with espilon, Normative
eps = 10
corrected_1n = one_sample_gc_correct(reads2.5K_n2,preds1n[1:l_1n],eps)
uncorrected_1n = one_sample_gc_correct(reads2.5K_n2,rep(1,length(reads2.5K_n2)),eps)

# One Sample with espilon, Tumor
eps = 10
corrected_1t = one_sample_gc_correct (reads2.5K_t2,preds1t[1:l_1t],eps)
uncorrected_1t = one_sample_gc_correct(reads2.5K_t2,rep(1,length(reads2.5K_t2)),eps)
```

```{r}
par(mfcol = c(2,1),mar = c(0.5,5,2,2))
discontinuous_range = c(5000:6000,15000:16000)
plot(uncorrected_1t[discontinuous_range], 
...
plot(corrected_1t[discontinuous_range], 
     ylab = "After", xaxt = "n",
     ylim = c(0,2.5),cex = 0.2,pch=20)
abline(h=1,col=4, lwd = 1.5)
abline(v=1000,col=2,lt = 2, lwd = 1.2)

plot(uncorrected_1n[discontinuous_range], ylab = "Before", xaxt = "n",
     main = "GC Correction for Healthy Sample",
     ylim = c(0,2.5),cex = 0.2,pch=20)
abline(h=1,col=4, lwd = 1.5)
abline(v=1000,col=2,lt = 2, lwd = 1.2)
plot(corrected_1n[discontinuous_range], ylab = "After", xaxt = "n",
     ylim = c(0,2.5),cex = 0.2,pch=20)
abline(h=1,col=4, lwd = 1.5)
abline(v=1000,col=2,lt = 2, lwd = 1.2)
...
# Plot for Healthy
plot(corrected_1n[discontinuous_range], ylab = "Healthy",xaxt = "n",
     main = "After GC Correction ('One Sample')", 
     ylim = c(0,2), cex = 0.2, pch=20)
abline(h=1, col=4, lwd = 1.5)

# Plot for Tumor
plot(corrected_1t[discontinuous_range], ylab = "Tumor",xaxt = "n",
     ylim = c(0,2), cex = 0.2, pch=20)
abline(h=1, col=4, lwd = 1.5)
...
plot(uncorrected_1t[discontinuous_range], main = "What's The Purpose Of This One...?",
     ylim = c(0,3),cex = 0.2,pch=20)
abline(h=1,col=4)
plot(corrected_1n[discontinuous_range],
     ylim = c(0,3),cex = 0.2,pch=20)
abline(h=1,col=4)
...
```{r}
two_sample_w_gc = two_sample_correct_e(corrected_1t,corrected_1n,0.1)
two_sample_wout_gc = two_sample_correct_e(uncorrected_1t,uncorrected_1n,0.1)
