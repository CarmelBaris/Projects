...
selected_GC_20K <- GC_20K_clean[start_idx:end_idx]

# Estimate the number of copies using the B-spline model
predicted_reads <- predict(reg_bs_train_div2, newdata = data.frame(GC_20K = selected_GC_20K))

# Show the marginal distribution
hist(predicted_reads, main = "Marginal Distribution of Predicted Reads", xlab = "Predicted Reads")

# Show the genome-wide distribution
genome_wide_predicted_reads <- predict(reg_bs_train_div2, newdata = data.frame(GC_20K = GC_20K_clean))
hist(genome_wide_predicted_reads, main = "Genome-wide Distribution of Predicted Reads", xlab = "Predicted Reads")

# Find a numerical index measuring closeness to 1
mse <- mean((predicted_reads - 1)^2)
mse
