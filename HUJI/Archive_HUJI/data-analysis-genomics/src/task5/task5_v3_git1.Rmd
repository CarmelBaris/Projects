...
# Division 1
proportions_div1 = check_proportions(train_div1, test_div1, 14/20)
...
# Division 2
proportions_div2 = check_proportions(train_div2, test_div2, 0.7)
...
# Division 3
proportions_div3 = check_proportions(train_div3, test_div3, 0.7)
...
# Division 1
overlaps_div1 = check_overlaps(train_indices_div1, test_indices_div1)
paste0("Has Div1 passed overlap test? ", overlaps_div1)

# Division 2
overlaps_div2 = check_overlaps(train_indices_div2, test_indices_div2)
paste0("Has Div2 passed overlap test? ", overlaps_div2)

# Division 3
overlaps_div3 = check_overlaps(train_indices_div3, test_indices_div3)
paste0("Has Div3 passed overlap test? ", overlaps_div3)

# Check Data Integrity
check_data_integrity = function(train, test, original_data) {
  total_train_test = length(train) + length(test)
  original_length = length(original_data)
  total_train_test == original_length  # Should be TRUE if the data integrity is maintained
}
...
# Division 1
data_integrity_div1 = check_data_integrity(train_div1, test_div1, reads_20K)
print(data_integrity_div1)

# Division 2
data_integrity_div2 = check_data_integrity(train_div2, test_div2, reads_20K)
print(data_integrity_div2)

# Division 3
data_integrity_div3 = check_data_integrity(train_div3, test_div3, reads_20K)
...
fit_spline_ns = function(train_data, test_data, knots) {
  ...
  predicted_train = predict(spline_model, newdata = data.frame(x_train_scaled = x_train_scaled))
  predicted_test = predict(spline_model, newdata = data.frame(x_train_scaled = x_test_scaled))
  
  # Calculate fit metrics
  metrics_train = fitMetrics(train_data, predicted_train)
  metrics_test = fitMetrics(test_data, predicted_test)
  
  list(model = spline_model, predicted_train = predicted_train, predicted_test = predicted_test, metrics_train = metrics_train, metrics_test = metrics_test)
}
```
...
plot_results_ns = function(train_data, test_data, predicted_train, predicted_test, title) {
  n = length(train_data)
  x_train = 1:n
  x_test = (n + 1):(n + length(test_data))
  
  plot(1:(length(train_data) + length(test_data)), c(train_data, test_data), type = 'n', main = title, xlab = "Index", ylab = "Read Counts", ylim = range(c(train_data, test_data, predicted_train, predicted_test), na.rm = TRUE))
  lines(x_train, train_data, col = 'blue', lwd = 2)
  lines(x_test, test_data, col = 'green', lwd = 2)
  lines(x_train, predicted_train, col = 'red', lwd = 2, lty = 2)
  lines(x_test, predicted_test, col = 'orange', lwd = 2, lty = 2)
  legend("topright", legend = c("Train Data", "Test Data", "Predicted Train", "Predicted Test"), col = c("blue", "green", "red", "orange"), lty = c(1, 1, 2, 2), lwd = 2)
}

plot_results_ns(train_div1, test_div1, spline_div1_ns$predicted_train, spline_div1_ns$predicted_test, "Division 1: Natural Spline Regression")
plot_results_ns(train_div2, test_div2, spline_div2_ns$predicted_train, spline_div2_ns$predicted_test, "Division 2: Natural Spline Regression")
plot_results_ns(train_div3, test_div3, spline_div3_ns$predicted_train, spline_div3_ns$predicted_test, "Division 3: Natural Spline Regression")
```
...
## Identify outliers from datasets of reads count and GC content
...
reg_2 = lm(reads_20K_clean ~ GC_20K_clean)
reg_3 = lm(reads_20K ~ GC_20K, subset = clean_subset)
reg_4 = lm(reads_20K_clean ~ GC_20K_clean, subset = clean_subset)
