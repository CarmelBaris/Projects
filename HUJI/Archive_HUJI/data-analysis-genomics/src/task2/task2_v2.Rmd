...
# Histogram graphics
  ...
  formatLabels <- function(x) {
    if (x >= 1e06) {return(paste0(round(x / 1e06, 2), "M"))}
    if (x >= 1e03) {return(paste0(round(x / 1e03, 1), "K"))} 
...
Clearly, the vast majority of bins have only 1 read, or no reads at all. \n


```{r getLine_func, message=FALSE, warning=FALSE}

getBinnedLine = function(reads_line, last_read, bin_size) {
  
  # Initialize the vector to aggregate read counts in bins of 10K
  # Ttl = length(reads_line+1)
  # last_read = reads_line[Ttl]
  # Nbins = ceiling(length(reads_line) / bin_size)
  Nbins = ceiling(as.numeric(last_read)/20000)
  bin_line = numeric(Nbins) # vector size is 10K times smaller

  # Iteratively add locations to respective bins
  # for (r in 1:Ttl) {
  for (r in 1:flast_read){
      j = 1 + (floor(reads_line$Loc[r] - 1) / bin_size)
    bin_line[j] = bin_line[j] + 1
  }
  print(object.size(bin_line))
  return(bin_line)
}
...
```{r message=FALSE, warning=FALSE, include=FALSE}
  ...
  # Binning full data file to bins of 10K
  reads_20K = getBinnedLine(chr1_reads$Loc,1)

  # Binning 20M reads to bins of 10K
  reads_20K_zoom = getBinnedLine(reads_20M,2e+04)
  

```

By clustering the data into bins of 20K reads each, we achieve an aggregated view of the marginal distribution. \n

```{r}
# find outlier cutoffs:
h <- hist(pmin(reads_20K,1000),breaks = 100,plot = FALSE)
...
text(x=bottom_cut-10,y=2000, 
     labels="Cutoff of <20",
     bg="transparent",col = "red", cex = 0.7, srt=90,pos=3)
text(x=top_cut-10,y=2000, 
     labels="Cutoff of >200",
     bg="transparent",col = "red", cex = 0.7, srt=90,pos=3)
...
Yet in the center of the histogram, we now recognize a semi bell-curve distribution with what seems to be a double-hump. \n

Seeing as the out seen to mask the central distribution, we will exclude them and further analyze the remaining bins. \n

Marked in red are the cutoff values of an average coverage below 0.002 and over 0.06 reads per bin. \n
...
no_outlier = reads_20K > bottom_cut & reads_20K < top_cut 
...
segments(x0=m_no_out,x1=m_no_out,y0=0,y1=y_max-40,col="darkblue",lw=2)
...
By filtering out records of extreme read counts per bin, the double-hump is more evident. \n
...
The fitted normal distribution (red reads_20M) does not match the histogram well.
