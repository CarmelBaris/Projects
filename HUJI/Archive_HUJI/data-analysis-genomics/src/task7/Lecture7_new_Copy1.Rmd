...
load(sprintf("%s/reads_100_A1.rda",dir_name)) #t(umor)
load(sprintf("%s/reads_100_B1.rda",dir_name)) #n(ot)
load(sprintf("%s/GC_100.rda",dir_name))
```

```{r t_reads}
#nr==nrow==limit_num_of_rows
chr1t_mat = matrix(reads_100_A1, nr = 50) # limit row number to 50
dim(chr1t_mat)
chr1t_mat[,1:10] # first 10 columns
reads_100_A1[1:2] # first two elements from cancerous sample
...
reads_100_B1[1:2] # first two elements in healthy sample
...
abline(h=median(reads_100_A1), col="tan2")
