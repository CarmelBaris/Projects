...
```{r fit}
f = function(X, Y) {
  lm(Y ~ bs(X, degree = 3, knots = c(0.415, 0.45, 0.54), Boundary.knots = c(min(X), max(X))))
}
...
reads2.5K_n2_scaled = scales::rescale(reads2.5K_n2_clean, to = c(0, 100))
...
plot(GC2.5K_clean[samp1], reads2.5K_n2_scaled[samp1], cex = 0.4, pch = 20, ylab = "Healthy 2", ylim = c(1.5, 35), xlim = c(0.2, 0.75))
plot(GC2.5K_clean[samp1], reads2.5K_t2_scaled[samp1], cex = 0.4, pch = 20, ylab = "Tumor 2", ylim = c(0, 35), xlim = c(0.2, 0.75))

```
### Plotting healthy vs tumorous samples
```{r}
plot(reads2.5K_n2[samp1], reads2.5K_t2_scaled[samp1], cex = 0.4, pch = 20, ylim = c(0, 35), xlim = c(0, 35), ylab = "Healthy Sample", xlab = "Tumor Sample")
```
...
reads_n = reads2.5K_n2
mod_2n = lm(reads2.5K_n2~bs(GC, degree = 3),subset = reads2.5K_n2>1.5 & reads2.5K_n2<35 )
...
plot(GC2.5K_clean[samp1],reads2.5K_n2[samp1],ylim = c(0,35),xlim = c(0.2,.75), cex = 0.5, pch=20, main = "Healthy 2")
lines(pred_range, resp_n,col = 4,lw=3)
lines(pred_range, resp_n,col = 4,lw=3)

```

### Fitting for tumor sample
...
```{r med_fix}
corrected_2n = one_sample_correct (reads2.5K_n2,preds2n[1:l_2n])
uncorrected_2n = (reads2.5K_n2)/median(reads2.5K_n2) 
...
l_2n = length(reads2.5K_n2)
GC = GC2.5K_clean[1:l_2n]
residuals_2n = reads2.5K_n2 - preds2n
```

```{r}
### Median Correction
fixed_2n = reads2.5K_n2 / median(reads2.5K_n2)

### Variance Decomposition
total_variance = var(reads2.5K_n2)
poisson_variance = mean(reads2.5K_n2)/(median(reads2.5K_n2)^2)
...
percentage_poisson+percentage_gc+percentage_unexplained == 100
```
...
reads2.5K_t2_scaled = reads2.5K_t2 / median_t2
reads2.5K_n2 = reads2.5K_n2 / median_n2

# Create a data frame for the results
results <- data.frame(
  Bin = 1:length(reads2.5K_t2),
  Reads_T2 = reads2.5K_t2,
  Reads_T2_Normalized = reads2.5K_t2_scaled,
  Reads_N2 = reads2.5K_n2,
  Reads_N2_Normalized = reads2.5K_n2,
  GC_Content = GC2.5K
)
