...
In DNA-sequencing, the uniform process is discrete (the reads can be in locations
...
s_dat = sort(unif_dat)
k = 5
upto5 = binReads(ceiling(s_dat), k)
abov5 = binReads(ceiling(s_dat), k)
hist(upto5,(0:12)-0.5,freq = FALSE)

points(0:10, dpois(lambda = 1.0,x = 0:10))
```

We get the Poisson by counting occurrences in equal-size intervals, so on average we will have one occurrence per bin. 
Here we take an interval of size $k$.
...
```{r}
k = 1
output = binReads(ceiling(s_dat[1:1000000]), k)
...
tic()
first_read = min(which(chr1_reads_orig$Loc >= beg_region)) 
r = first_read
tt = proc.time()
while (chr1_reads_orig$Loc[r] <= end_region){
  read_line[chr1_reads_orig$Loc[r]] = read_line[chr1_reads_orig$Loc[r]]+1
  r = r+1
}
toc()
...
```{r}
nreads = nrow(chr1_reads_orig)
last_read = chr1_reads_orig$Loc[nreads]

# Last bin
N5K = ceiling(last_read/5000)

reads_5K= numeric(N5K)
for (r in 1:nrow(chr1_reads_orig)){
  # Need to check such dangerous lines...
  bin5K = 1+floor((chr1_reads_orig$Loc[r]-1)/5000) 
  reads_5K[bin5K] = reads_5K[bin5K]+1 
}
# Check that these are correct
reads_5K[1]
chr1_reads_orig[reads_5K[1]+c(-10:10),]
```
