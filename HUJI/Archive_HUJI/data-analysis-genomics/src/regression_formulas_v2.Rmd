...
\Omega(L_j) = \{ {i} \}\\
Y_i = \sum_{i=1}^{I} \big[𝟙_{L_j=i} \big] \\
...
```{r}
par(mfrow=c(2,1))
plot(reads_2.5K_n1, main = "Marginal Distr. of Read Count in Healthy Sample", ylim=c(0,600))
...
$$
Y_K = \text{Count of Reads in bin K}\\
X_K=\text{GC Content (%) in bin K}\\
K \text{ is the serial number of a given bin}\\
K = 1,2,\cdots,\frac{\ I}{B} \\
\text{ (where } B \text{ is the bin size)} \\
\Omega(X_K) = \Omega(X_i) = \big\{0,\cdots,1\big\} \\
\Omega(Y_K) = \Omega(Y_i) = \big\{0,\cdots,J\big\} \\
\Omega(L_j') = \big\{ K \big\} \\
...
$$ RMSE( \{ \hat{a}{_{_k}} \}, \vec{\mathbf{1}}) = \sqrt{\frac{1}{b} \sum_{k}^{} \Big(\hat{a}{_{_k}} - \vec{\mathbf{1}}\Big)^2} \\
{\mathbb{E}}MSE( \{ \hat{a}{_{_k}} \}, \vec{\mathbf{1}}) 
  = \mathbb{E}_Y \bigg[ MSE\Big( \{ \hat{a}{_{_k}} \},\vec{\mathbf{1}} \Big) \bigg] 
  = \mathbb{E}_Y \bigg[\frac{1}{b} \sum_{k}^{} \Big(\hat{a}{_{_k}} - \vec{\mathbf{1}}\Big)^2 \bigg]\\
  = \frac{1}{b} \sum_{k}^{} \mathbb{E}_Y \bigg[\Big(\hat{a}{_{_k}} - \vec{\mathbf{1}}\Big)^2 \bigg]\\
\mathit{dist} = 
...
Our task is to categorize this DNA sample as either normative ("Healthy") or cancerous ("Tumor").
...
\textbf{What do we expect, if sample } s \textbf{ is healthy?} \\
    \frac{Y^s}{Y^h} =
\bigg(
    \frac{\text{Num. of Reads from Suspected Sample }s}
         {\text{Num. of Reads from Healthy Sample }h} 
            \bigg) 
    \overset{\checkmark}{=} 1
    \\
$$
...
\textbf{What do we expect, if sample s is healthy?} \\
    \frac{Y^s}{Y^h} 
      \overset{\lambda_k \sim Unif}{\propto} \frac{Y^s}{{\mathbb{E}_k[Y^h]}} 
      \approx \frac{Y^s}{{\mathbb{\hat{E}}_k[Y^h]}} 
      =\frac{Y^s}{\operatorname{med}(Y^h)} 
    \overset{?}{=} 1
    \\
$$
...
\textbf{What do we expect, if sample s is healthy?} \\
    \frac{Y^s}{Y^h} 
      \approx \frac{Y^s}{{\mathbb{E}[Y^h]}} 
      \approx \frac{Y^s}{{\mathbb{\hat{E}}[Y^h]}} 
      =\frac{Y^s}{\operatorname{median}(Y^h)} 
    \overset{?}{=} 1
    \\
$$


We expect to see changes depending on the location of the bin. \n
Supposedly $Y_K \sim \text{(Location of Bin K)$, but we know this is not true! \n
If we take GC content into account, the actual read count in bin K is $\ Y_K \sim f(\text{GC}{_{K}}) \cdot \mathcal{E}_{K}$. \n
And then our estimated read count in bin K is $\hat{Y}_K \sim \hat{f}(\text{GC}_{K})$.



### Lab 6
$$
{Y_k}^n \ \text{ Healthy sample (Normative)} \\
...
\text{This means that we expect } \eta^h / \eta^t \approx 1
$$

After recess:

$$
\hat{a}_k^{(3.0)} \text{ This is a two-sample estimator that takes GC content into account}\\
\text{This means that we expect } \eta^h / \eta^t \approx 1 \\
...
\text{Problems with estimators that are a if the denominator is close to 0.}\\
\\
\hat{a}_k^{(3.0)} = \frac{}{\frac{{Y_k}^n}{{\hat{f}_k}^n{GC}_k}+\mathcal{E}}
$$
