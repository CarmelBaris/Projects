...
dir_help = "C:/Users/user/Documents/Carmel/Projects/HUJI/Archive_HUJI/data-analysis-genomics/data/"
# dir_help = "C:/Users/user/Documents/Carmel/Projects/HUJI/Archive_HUJI/data-analysis-genomics/task4/"
...
helpers_file = file.path(dir_help, "help_funcs_v4_STABLE.R")
source(helpers_file)
helpers <- list(
    loadRData = loadRData,
    fitMet = fitMetrics,
...
plot(GC_20K_clean, reads_20K_clean, pch = 20, cex = 0.5, col = rgb(0,0,0,0.6),
...
legend("topleft", legend = c("Segmented Package", "Manual Piecewise"), 
       col = c("purple", "orange"), lwd = 2, cex = 0.7, bty = "n")
...
pred_lin = predict(initial_model)
...
## Bias Assessment
The goal is to have residuals that are evenly distributed around zero across all GC values. In the segmented model plotted on the left, the residuals are scattered randomly around zero, without any specific trend or pattern. Yet as seen in the residuals plot on the right-hand side, there is a noticeable pattern for GC values higher than 49.6%, indicating a bias. At circa 50-52% the model seems to systematically over-predict the read count, whereas above 52% the model tends to under-predict. 
