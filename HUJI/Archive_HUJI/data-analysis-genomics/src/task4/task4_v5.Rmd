...
```{r pw_regression}
# Extract data
GC_5K_clean <- reads_gc_5K$GC_5K
reads_5K_clean <- reads_gc_5K$reads_5K

# Fit initial linear model to find breakpoints
...
# Fit piecewise polynomial regression with quadratic polynomials
pw_poly_model <- lm(reads_5K_clean ~ I((GC_5K_clean - knot1)^2 * (GC_5K_clean < knot1)) +
                                   I((GC_5K_clean - knot1) * (GC_5K_clean < knot1)) +
                                   I((GC_5K_clean - knot2)^2 * (GC_5K_clean >= knot1 & GC_5K_clean < knot2)) +
                                   I((GC_5K_clean - knot2) * (GC_5K_clean >= knot1 & GC_5K_clean < knot2)) +
...
Let's start by comparing the residuals of the piecewise polynomial model and the linear model.

```{r}
# Calculate residuals
pw_poly_residuals <- reads_5K_clean - predict(pw_poly_model)
linear_residuals <- reads_5K_clean - predict(linear_model)
