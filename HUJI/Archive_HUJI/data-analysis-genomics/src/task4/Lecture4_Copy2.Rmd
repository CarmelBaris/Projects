...
$$ \textit{e}_i \text{Part of this text is written } \textit{italic} $$

Again, we need to rescale (by truncating the y axis) so we can see something
```{r reg, results='asis'}
plot(rnorm(100), main = substitute(paste(italic('e')[i], " = 0.01")))
mysubtitle1 = expression(paste("Residuals  ",plain(cos) * phi, e[1]))
mtext(side = 3, line = 0.25, at = 1, adj = -1.5, mysubtitle1)
...
cool=TeX(r'(${e}_i  x^\alpha$, where $\alpha \in \{1 \ldots 5\}$)')
x <- seq(0, 4, length.out=100)
alpha <- 1:5
mymain = TeX(r'(Non-Std. Residuals $\textit{e}_i$ vs. Explanatory Variable $X_j$)', bold=TRUE)
plot(x, xlim=c(0, 4), ylim=c(0, 10), xlab='x', ylab=cool, type='n', main="")
title(TeX(r'(Non-Std. Residuals $e_i$ vs. Explanatory Variable $X_j$)', bold=TRUE))

mymain1 = "Scatter Plot of Residuals"
mymain2 = " vs. single explanatory variable"
mysubtitle = "Residuals from Linear Regression of GC-Richness vs." 
