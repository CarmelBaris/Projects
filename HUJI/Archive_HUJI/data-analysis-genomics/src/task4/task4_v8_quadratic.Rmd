...
pw_quadratic_model <- lm(reads_5K_clean ~ I((GC_5K_clean - knot1)^2 * (GC_5K_clean < knot1)) +
                                     I((GC_5K_clean - knot2)^2 * (GC_5K_clean >= knot1 & GC_5K_clean < knot2)) +
                                     I((GC_5K_clean - knot2)^2 * (GC_5K_clean >= knot2)))

# Predict and plot the piecewise quadratic model
new_data_quad <- data.frame(GC_5K_clean = seq(min(GC_5K_clean), max(GC_5K_clean), length.out = 100))
predictions <- predict(pw_quadratic_model, newdata = new_data_quad)
...
new_data_lin <- data.frame(GC_5K_clean = seq(min(GC_5K_clean), max(GC_5K_clean), length.out = 100))
predictions <- predict(pw_linear_model, newdata = new_data_lin)
...
pw_quad_residuals <- reads_5K_clean - predict(pw_quadratic_model)
pw_linear_residuals <- reads_5K_clean - predict(pw_linear_model)
...
plot(GC_5K_clean, pw_linear_residuals, pch = 16, cex = 0.5, col = rgb(0, 0, 0, 0.5),
     xlab = "GC Content (cleaned)", ylab = "Residuals",
     main = "Residuals of Piecewise Linear Model")

# Plot residuals for segmented model
plot(GC_5K_clean, pw_quad_residuals, pch = 16, cex = 0.5, col = rgb(0, 0, 0, 0.5),
     xlab = "GC Content (cleaned)", ylab = "Residuals",
...
Now, let's calculate and compare RMSE and R-squared values for both models.
...
# Calculate RMSE for piecewise polynomial model
pw_poly_rmse <- rmse(pw_poly_residuals)

# Calculate RMSE for linear model
linear_rmse <- rmse(pw_linear_residuals)
poly_rmse <- rmse(pw_poly_residuals)
quad_rmse <- rmse(pw_quad_residuals)

# Calculate RMSE for quadratic model
linear_rmse <- rmse(pw_quad_residuals)

# Calculate R-squared for piecewise polynomial model
pw_poly_r_squared <- r_squared(pw_quadratic_model, GC_5K_clean, reads_5K_clean)

# Calculate R-squared for linear model
linear_r_squared <- r_squared(pw_linear_model, GC_5K_clean, reads_5K_clean)

# Print metrics
cat("RMSE of Piecewise Polynomial Model:", pw_poly_rmse, "\n")
cat("RMSE of Linear Model:", linear_rmse, "\n", "\n")
...
The RMSE and R-squared values further support that the piecewise polynomial model provides a better fit to the data compared to the linear model.
