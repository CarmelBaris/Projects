...
# Median & GC Fix (Tumor Sample)
corrected_1t = fix_med(fix_gc_e(reads_tumor, preds_tumor_bs,0.3))
corrected_1n = fix_med(fix_gc_e(reads_healthy, preds_healthy_bs,0.3))

# Median & Two-Sample Fix (No GC Fix)
...
# Median & GC & Two-Sample Fix
for_gc_2samp_healthy = fix_gc_e(reads_healthy, preds_healthy_bs)
for_gc_2samp_tumor = fix_gc_e(reads_tumor, preds_tumor_bs)
...
rect(a_bins[1], 0.25, a_bins[2], 2.2, col = rgb(0,1,0,0.2), border = NA) 
...
# Highlight the ranges with a shaded region
rect(a_bins[1], 0, a_bins[2], 2, col = rgb(0,1,0,0.2), border = NA) 
rect(b_bins[1]-10, 0.25, b_bins[2]+30, 2.2, col = rgb(1,0,0,0.2), border = NA) 
...
# Highlight the ranges with a shaded region
rect(a_bins[1], 0.35, a_bins[2], 2, col = rgb(0,1,0,0.2), border = NA) 
rect(b_bins[1]-10, 0.25, b_bins[2]+30, 2.2, col = rgb(1,0,0,0.2), border = NA) 
