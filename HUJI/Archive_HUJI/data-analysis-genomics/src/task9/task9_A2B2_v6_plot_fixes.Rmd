...
outliers are commented out, so none are removed.
...
r_health_cln = reads2.5K_n2
r_tumor_cln = reads2.5K_t2
GC_cln = GC2.5K
...
```{r colors}
col_healthy = rgb(0.8, 0.8, 0.8, 0.6)
color_tumor = rgb(0.3, 0.3, 0.3, 0.4)
```
...
plot(GC_healthy, reads2.5K_t2, col = color_tumor, pch = 20, cex = 0.5, ylab = "Binned Reads (2,500 per bin)", xlab = "GC Content", main = "Healthy vs Tumor Sample", ylim = c(0,400))
points(GC_healthy, reads2.5K_n2, col = col_healthy, pch = 20, cex = 0.5)
...
# Add legend
legend("topleft", legend = c("Tumor Sample", "Healthy Sample"), col = c(color_tumor, col_healthy), pch = 20, bty = "n")
...
### Plotting Fixes in Healthy Sample

```{r plot_fixes_1samp_h}
par(mfcol = c(3,1),mar = c(2,5,2,2), oma = c(3,4,3,4))
...
abline(v=c(1200,1500,4000),col=c("orange","orange",6), lt = c(1, 1, 2), lwd = c(1.4, 1.4, 1.3))
...
mtext("Est. Number of Copies (a hat)", outer = TRUE, side = 2, cex = 0.65, adj = 0.2)
mtext("Reads Ct.", outer = TRUE, side = 2, cex = 0.65, adj = 0.9)
...
```{r plot_t_1samp2samp}
par(mfrow=c(2,2), mar = c(1.5,2.5,2.5,2.5), oma = c(3,4,3,4))
...
plot(unfixed_2t_gc_med_eps[stiched_range], 
     ...
     cex = 0.6, pch=20, col = col_healthy
     )
...
plot(med_2samp[stiched_range],
     ylab = "", xaxt = "n",
     ylim = c(0,2), cex = 0.5, pch=20, col = col_tumor)
abline(h=1,col=4, lwd = 1.5)
abline(v=c(1200,1500,4000),col=c("orange","orange",6), lt = c(1, 1, 2), lwd = c(1.4, 1.4, 1.3))
