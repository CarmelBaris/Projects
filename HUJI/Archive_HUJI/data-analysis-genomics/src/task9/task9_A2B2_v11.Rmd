...
# rm("GC2.5K", "reads_healthy", "reads_tumor") # remove from memory
```
...
# Calculate the mean of the data (lambda for Poisson)
lambda <- mean(df$reads_tumor)
...
# Calculate the Poisson probabilities for the theoretical distribution
max_bin <- max(df$reads_tumor)
poisson_df <- data.frame(x = 0:max_bin, prob = dpois(0:max_bin, lambda))
...
p <- ggplot(df_hist, aes(x = mids, y = counts)) +
  ...
  geom_line(data = poisson_df, aes(x = x, y = prob * sum(read_hist$counts)), color = "blue", size = 1) +
  labs(title = "Histogram of Occurrences Counts with Poisson Distribution",
       x = "Count of Fragments Per Bin",
       y = "Frequency",
       caption = paste("Mean coverage:", round(lambda, 2))) +
  theme_minimal() +
...
# mod_2n = f(GC, reads_healthy, subs = subs_n)
# mod_2t = f(GC, reads_tumor, subs = subs_n)
mod_2n = lm(reads_healthy~bs(GC, degree = 3),subset = subs_n)
mod_2t = lm(reads_tumor~bs(GC, degree = 3),subset = subs_t)

resp2n = predict(mod_2n,list(pred_range))
preds_2n = predict(mod_2n,list(GC))

resp2t = predict(mod_2t,list(pred_range))
preds_2t = predict(mod_2t,list(GC))
...
# Tumor Sample
reads_tumor_cl = reads_tumor[1:l_2n]  # Ensure tumor data matches in length
...
preds_tumor = predict(mod_tumor, newdata = data.frame(GC = pred_range))
...
lines(pred_range, preds_tumor, col = 'mediumvioletred', lwd = 2)
