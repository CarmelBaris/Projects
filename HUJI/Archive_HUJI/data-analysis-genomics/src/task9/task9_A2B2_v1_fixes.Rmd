...
two_samp_med_epsone = fix_med(fix_2samp_e(gc_unfixed_tumor,gc_unfixed_healthy,1))
two_samp_med_epsten = fix_med(fix_2samp_e(gc_unfixed_tumor,gc_unfixed_healthy,10))
two_samp_gc_med_eps = fix_med(fix_2samp_e(gc_fixed_tumor,gc_fixed_healthy,eps))
...
```{r}
par(mfcol = c(2,1),mar = c(5,5,5,5))

## two sample median + GC correction
plot(fixed_2t_gc_med_eps[stiched_range], 
     main = "Tumor: One-Sample vs Two Sample",
     xlab = "Selected Locations in Chrom1 (25M-35M, 75M-80M)",
     ylab = "One Sample", xaxt = "n",
     ylim = c(0,2),cex = 0.2,pch=20, xlim = c(0,6000))
abline(h=1,col=4, lwd = 1.5)
abline(v=4000,col=6,lt = 2, lwd = 1.2)
mtext("Incl. Median & Fixes (eps = 0.1)", side = 3, cex = 0.8)

plot(two_samp_gc_med_eps[stiched_range], 
     ylab = "Two Sample", xaxt = "n", xlab = "",
     ylim = c(0,2),cex = 0.2,pch=20, xlim = c(0,6000))
abline(h=1,col=4, lwd = 1.5)
abline(v=4000,col=6,lt = 2, lwd = 1.2)
...
plot(two_samp_med_eps0.3[stiched_range],
     main = "Combining GC & Two-Sample (Epsilon = 0.1)",
