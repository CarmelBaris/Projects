...
# Evenly Spaced Knots
n_knots = 3
knots = seq(min(GC), max(GC), length.out = n_knots + 2)[-c(1, n_knots + 2)]
```
...
# Plot Tumor Sample with B-Spline Regression
plot(GC[samp1], reads_healthy[samp1], cex = 0.4, pch = 20, col = col_healthy,
...
plot(med_gc_2samp_eps0.3[stiched_range], 
     ...
     ylim = c(0,2), xlim = c(0,6000),
     cex = 0.45, pch=20, col = col_healthy)
grid(lwd = 0.3) # Add a grid for better readability
abline(h=1, col="royalblue", lwd = 3)
abline(v=4000, col="darkblue", lt = 2, lwd = 1.4)
abline(v=c(1420,1470), col="indianred2", lwd = 1.4)
...
plot(GC, reads_tumor, xlim = c(0, 0.7), ylim = c(0, 500))

# Logical condition to identify points within the specified range
dots <- reads_tumor > 50 & reads_tumor < 400
highlight_outside <- !(dots)

# Plot points within the specified range (without highlighting)

# Highlight points outside the specified range
points(GC[highlight_outside], reads_tumor[highlight_outside], col = "red", pch = 19)
```
...
grid() # Add a grid for better readability
abline(h = 1, col = "royalblue", lwd = 3)
abline(v = 4000, col = "darkblue", lty = 2, lwd = 1.4)
abline(v = c(1420, 1470), col = "indianred2", lwd = 1.4)
# Highlight the range 1420 to 1470 with a shaded region
rect(1420, 0, 1470, 2, col = rgb(1, 0, 0, alpha = 0.2), border = NA)
mtext("Median (eps = 0)", side = 3, cex = 1, col = col_tumor, line = 0.5)
...
range_start <- 1420
range_end <- 1470
subset_med_eps0 <- med_eps0[range_start:range_end]
subset_med_gc_2samp_eps0.3 <- med_gc_2samp_eps0.3[range_start:range_end]

# Calculate the means
mean_med_eps0 <- mean(subset_med_eps0, na.rm = TRUE)
mean_med_gc_2samp_eps0.3 <- mean(subset_med_gc_2samp_eps0.3, na.rm = TRUE)

# Print the means
mean_med_eps0
mean_med_gc_2samp_eps0.3
```
