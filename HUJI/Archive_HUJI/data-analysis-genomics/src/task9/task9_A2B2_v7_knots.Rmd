...
GC_cln_nonzero <- GC_cln[GC_cln != 0]
knots_list_nonzero <- create_knots(GC_cln_nonzero)

# Exclude GC values smaller than 0.3
GC_cln_filtered <- GC_cln[GC_cln >= 0.3]
knots_list_filtered <- create_knots(GC_cln_filtered)

# Example usage:
GC_cln <- runif(100, 0, 1)  # Replace with your actual data
...
mod_2n = f(GC_cln_nonzero, r_health_cln, 3, knots_list[[1]])
# mod_2n = f(GC, r_health_cln, 3, knots_list[[1]], subs_n)
mod_2t = f(GC_cln_nonzero, r_tumor_cln, 3, knots_list[[1]])
# mod_2t = f(GC, r_tumor_cln, 3, knots_list[[1]], subs_t)

# Make predictions
predict_data = data.frame(GC_cln_nonzero = seq(0, 0.7, 0.01))
predict_data$resp2n = predict(mod_2n, predict_data)
predict_data$resp2t = predict(mod_2n, predict_data)
preds_2n = predict(mod_2n, list(GC_cln = GC_cln_nonzero))
preds_2t = predict(mod_2t, list(GC_cln = GC_cln_nonzero))
...
# Plotting function
# plot_fits = function(GC_cln_nonzero, r_health_cln, r_tumor_cln, knots_list) {
plot_fits = function(GC, r_health_cln, r_tumor_cln, knots_list, pred_range) {
  par(mfrow = c(1, 2), mar = c(5, 4, 4, 2) + 0.1)
  
  # Plot for healthy sample
  plot(GC_cln_nonzero, r_health_cln, main = "Healthy Sample", xlab = "GC", ylab = "Reads")
  for (knots in knots_list) {
    subs_n = (r_health_cln > 50) & (r_health_cln < 350)
    mod_2n = f(GC_cln_nonzero, r_health_cln, 3, knots)
  ...
  for (knots in knots_list) {
    subs_t = (r_tumor_cln > 50) & (r_tumor_cln < 350)
    mod_2t = f(GC_cln_nonzero, r_tumor_cln, 3, knots)
...
plot_fits(GC_cln_nonzero, r_health_cln, r_tumor_cln, knots_list, pred_range)
# plot_fits(GC_cln_nonzero, r_health_cln, r_tumor_cln, knots_list)
...
# Set up the plotting area with adjusted margins
  ...
  for (knots_vec in knots_list) {
    ...
    points(GC, preds_2t, col = "green", pch = 16)
  }
}
...
### Chatgpt Knots Selection using K-Fold
